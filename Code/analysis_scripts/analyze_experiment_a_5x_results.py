"""
实验A统计分析: 5× Load结构对比 (n=3)
Statistical Analysis for Experiment A: 5× Load Structural Comparison (n=3)

目标:
- 分析5× load下的结构对比 (Inverted vs Normal Pyramid)
- 计算统计量 (Welch's t-test, 95% CI, Cohen's d)
- 生成LaTeX表格用于论文

注意:
- 数据路径: Data/ablation_studies/structural_5x_load/
...
def load_experiment_results_5x(config_type, algorithm, seeds=[42, 123, 456]):
    ...
    - algorithm: 算法名称 (A2C 或 PPO)
    - seeds: 随机种子列表 [42, 123, 456]
    ...
    # 所有seeds都来自structural_5x_load目录
    for seed in seeds:
        ...
        if result_path.exists():
            ...
            print(f"⚠️  警告: 未找到结果 {result_path}")
