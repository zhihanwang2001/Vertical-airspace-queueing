...
R2D2 implementation integrated into existing baseline algorithm framework
"""
...
class R2D2Baseline:
    """R2D2 baseline algorithm, compatible with existing framework"""

    def __init__(self, config: Dict[str, Any] = None):
        """
        Initialize R2D2 baseline
        ...
        default_config = {
            ...
            'recurrent_dim': 64, # Small RNN, suitable for short sequences
            'num_layers': 1,
            'recurrent_type': 'LSTM',
            'dueling': True,

            # Learning parameters - super aggressive learning
            'learning_rate': 1e-3,  # Very high learning rate, fast learning
            'gamma': 0.95,          # Lower discount, more focus on immediate rewards
            'target_update_freq': 500,   # Very frequent target network updates
            'gradient_clip': 5.0,   # Stricter gradient clipping to prevent explosion

            # DQN parameters - super fast exploration to exploitation transition
            'epsilon_start': 0.9,   # Lower initial exploration
            'epsilon_end': 0.1,     # Keep some exploration
            'epsilon_decay_steps': 10000,  # Faster decay
            'double_dqn': True,

            # Sequence replay configuration - extremely optimized for short episodes
            'buffer_size': 500,    # Smaller buffer, fast turnover
            'sequence_length': 3,   # Super short sequence, matches actual episode length
            'burn_in_length': 1,    # Minimal burn-in
            'overlap_length': 1,
            'batch_size': 32,       # Larger batch to compensate for short sequences

            # Training parameters - start learning immediately
            'learning_starts': 500,   # Very short wait, start learning quickly
            'train_freq': 2,          # Very frequent training, every 2 steps

            # Action discretization - key optimization!
            'action_bins': 2,  # Reduce to 2, lower action space complexity
    ...
    def train(self, total_timesteps: int, eval_freq: int = 10000, save_freq: int = 50000):
        ...
        while timestep < total_timesteps:
            ...
            if done:
                ...
                episode_length = 0
                state, _ = self.env.reset()
                # R2D2 will automatically reset hidden state when done=True
            else:
                state = next_state
    ...
    def evaluate(self, n_episodes: int = 10, deterministic: bool = True, verbose: bool = True):
        ...
        for episode in range(n_episodes):
            state, _ = self.env.reset()
            self.agent.reset_hidden_state()  # Reset RNN state
        ...
        results = {
            ...
            'system_metrics': []  # R2D2-specific metrics can be added here
        }
